'''
Method  Description:
I found that in the hybrid recommendation system I built in hw3, item-based cf is not
...
Besides, comparing to HW3, I add more features like tip, photo and check-in to the data.

To further improve performance, CatBoost was introduced and fine-tuned as an additional model. It is fast and robust.
It reduces the demand of fine-tuning hyperparameters, and reduce the chance of overfitting.

As result, I use a weighted combination of XGBoost and CatBoost predictions.
This approach leveraged their respective strengths, with XGBoost capturing user-item patterns,
and CatBoost addressing non-linear relationships and regularization.
Then, I optimize their weights to improve system performance and reduce RMSE.
...
def process_all_data(sc, file_path, user_dict, business_dict, tip_data, photo_counts_dict, checkin_counts_dict, mode='train'):
    """
    Combines data from multiple sources (user features, business features, user-business interactions,
