...
train_set = pd.read_csv('HW3StudentData/yelp_train.csv').rename(columns={'stars': 'target'})
eval_set = pd.read_csv('HW3StudentData/yelp_val.csv').rename(columns={'stars': 'target'})
test_set = pd.read_csv('HW3StudentData/yelp_test_ans.csv').rename(columns={'stars': 'target'})
...
with open("HW3StudentData/business.json", 'r', encoding='utf-8') as f:
...
with open("HW3StudentData/user.json", 'r', encoding='utf-8') as f:
    lines = map(json.loads, f.readlines())
...
with open('HW3StudentData/edges.txt', 'w+') as f:
    for line in data.values:
        f.write(f'{line[0]} {line[1]}\n')
    for pair in pairs:
        f.write(f'{pair[0]} {pair[1]}\n')

G = nx.read_edgelist('HW3StudentData/edges.txt',
...
df_embeddings = pd.DataFrame.from_dict(embeddings, orient='index',
    ...
    columns={'index': 'id'})
df_embeddings.to_csv('HW3StudentData/embeddings.csv', index=False)
